...
class Solver:
    ...
    def _compute_r(self):
        ...
        logger.info("Computing the distance from each point on the FW-H "
    ...
    def _discretize_observer_time(self):
        ...
        t_start = (np.min(self.source.time_domain)
                   + np.min(self.r) / self.config.solver.constants.c_0)
        t_end = (np.max(self.source.time_domain)
                 + np.max(self.r) / self.config.solver.constants.c_0)
        self.time_domain = np.linspace(t_start,
                                       t_end,
                                       self.config.solver.time_steps)
    ...
    def _compute_v_n(self):
        ...
        logger.info("Computing the normal velocity for each point on the FW-H "
    ...
    def _compute_v_n_dot(self):
        ...
        logger.info("Computing the source time derivative of the normal "
                    "velocity for each point on the FW-H surface over the "
                    "source time domain...")
        self.v_n_dot = np.gradient(self.v_n,
                                   self.source.time_domain,
                                   axis=0)

    def _compute_p_dot(self):
        """Compute the source time derivative of the pressure."""
        logger.info("Computing the source time derivative of the pressure for "
                    "each point on the FW-H surface over the source time "
                    "domain...")
        self.p_dot = np.gradient(self.source.fw_h_pressure,
                                 self.source.time_domain,
                                 axis=0)

    def _compute_cos_theta(self):
        """Compute cos(θ) for each point on the FW-H surface."""
        logger.info("Computing the cos(θ) for each point on the FW-H "
                    "surface...")
        self.cos_theta = (
    ...
    def compute(self):
        ...
        for i in range(len(self.source.time_domain)):
            indices = np.argmin(np.abs(
                self.time_domain[:, np.newaxis] - retarded_time[i, :]
            ), axis=0)
